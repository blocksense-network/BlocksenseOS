...
class SecurityAudit
  ...
  def audit_build_security
    ...
    if ci_files.any?
      ...
      ci_files.each do |ci_file|
        ...
        if ci_content.match?(/security|audit|vulnerability/i)
          log_success "Security checks found in #{ci_file}"
        else
          log_warning "No security checks in CI file: #{ci_file}"
          add_finding('medium', 'build_security', 'Missing CI security checks',
                     'CI pipeline lacks security scanning steps', ci_file)
        end
        
        # Check for secrets handling
        if ci_content.include?('secrets.')
          if ci_content.include?('GITHUB_TOKEN')
            log_success "Proper secrets handling in #{ci_file}"
          else
            log_warning "Custom secrets used in #{ci_file}"
            add_finding('medium', 'build_security', 'Custom secrets in CI',
                       'Review secret usage in CI pipeline', ci_file)
          end
        end
      end
    else
    ...
    if File.exist?('flake.nix')
      ...
      if flake_content.match?(/tpm|luks|security/i)
        ...
        log_warning "No security packages found in flake.nix"
        add_finding('low', 'build_security', 'Missing security packages',
                   'Consider adding security-focused packages to the build', 'flake.nix')
      end
    end
  end
  ...
  def audit_documentation
    ...
    if threat_doc
      ...
      threat_sections = ['Assets', 'Threat Actors', 'Attack Scenarios', 'Mitigations']
      
      threat_sections.each do |section|
