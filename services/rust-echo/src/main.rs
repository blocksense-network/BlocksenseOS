...
mod tests {
    ...
    async fn test_echo_functionality() {
        ...
        let test_message = b"Hello, echo service!";
        ...
        assert_eq!(response.trim(), "Hello, echo service!");
    }
    ...
    async fn test_large_message_handling() {
        ...
        stream.write_all(large_message.as_bytes()).await.unwrap();
    ...
    async fn test_message_structure() {
        ...
        let deserialized: EchoMessage = serde_json::from_str(&json).unwrap();
    ...
    async fn test_rate_limiter_garbage_collection() {
        ...
        assert!(rate_limiter.check_rate_limit("client2").await.is_err());

        // Wait for cleanup window
        tokio::time::sleep(Duration::from_secs(2)).await;

        // Should be able to make requests again (garbage collection cleaned up old entries)
        assert!(rate_limiter.check_rate_limit("client1").await.is_ok());
        assert!(rate_limiter.check_rate_limit("client2").await.is_ok());
    }

    #[tokio::test]
    async fn test_rate_limiter_memory_management() {
